...
export async function registerUser(input) {
  ...
  if (!name || !password) {
    throw httpError("Preencha nome e senha.");
  }
  ...
  if (existing.rows[0]) {
    throw httpError("Este nome ja tem cadastro.", 409);
  }

  const user = { id: uid(), name };
  const internalEmail = `${user.id}@lixo-eletronico.local`;
  await db.execute({
    sql: "INSERT INTO users (id, name, email, password) VALUES (?, ?, ?, ?)",
    args: [user.id, user.name, internalEmail, password],
  });
...
export async function loginUser(input) {
  ...
  const result = await db.execute({
    sql: "SELECT id, name FROM users WHERE name = ? COLLATE NOCASE AND password = ?",
    args: [name, password],
  });

  const user = result.rows[0] ? Object.fromEntries(Object.entries(result.rows[0])) : null;
  if (!user) {
    throw httpError("Nome ou senha invalido.", 401);
  }

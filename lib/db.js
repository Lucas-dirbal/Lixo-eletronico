...
export async function registerUser(input) {
  ...
  if (!name || !email || !password) {
    throw httpError("Preencha nome, e-mail e senha.");
  }
  ...
  if (existing.rows[0]) {
    throw httpError("Este e-mail ja tem cadastro.", 409);
  }

  const user = { id: uid(), name, email };
  await db.execute({
    sql: "INSERT INTO users (id, name, email, password) VALUES (?, ?, ?, ?)",
    args: [user.id, user.name, user.email, password],
  });

  await getOrCreateStudent(name);
  return user;
}
...
export async function loginUser(input) {
  ...
  if (!email || !password) {
    throw httpError("Preencha e-mail e senha.");
  }

  await initDb();

  const result = await db.execute({
    sql: "SELECT id, name, email FROM users WHERE email = ? COLLATE NOCASE AND password = ?",
    args: [email, password],
  });

  const user = result.rows[0] ? Object.fromEntries(Object.entries(result.rows[0])) : null;
  if (!user) {
    throw httpError("E-mail ou senha invalido.", 401);
  }

  await getOrCreateStudent(user.name);
  return user;
}
